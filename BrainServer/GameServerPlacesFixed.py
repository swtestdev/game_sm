...
def get_slide_for_places(pages):
    places = {"1 МЕСТО": None, "2 МЕСТО": None, "3 МЕСТО": None,
              "ЗОЛОТАЯ СЕРЕДИНА": None, "ЗАЩИТА ТЫЛА": None}
    for slide in pages:
...
def load_places():
    ...
    if game_data is None:
        ...
        if "None" not in str(game_score):
            btnGameLaoding.config(text="Update places")
        else:
            update_log_display("Not found all slides in the Game")
        ...
        if sheet.cell(row=1, column=1).value == '#' and sheet.cell(row=1, column=2).value == 'КОМАНДЫ':  # Название
            ...
            try:
                ...
                tk.messagebox.showinfo(title="PLACES SETUP NOTIFICATION",
                                       message=f"1 МЕСТО: {game_score['1 МЕСТО'].Text}\n"
                                               f"2 МЕСТО: {game_score['2 МЕСТО'].Text}\n"
                                               f"3 МЕСТО: {game_score['3 МЕСТО'].Text}\n"
                                               f"ЗОЛОТАЯ СЕРЕДИНА: {game_score['ЗОЛОТАЯ СЕРЕДИНА'].Text}\n"
                                               f"ЗАЩИТА ТЫЛА: {game_score['ЗАЩИТА ТЫЛА'].Text}")
            except Exception as e:
                print(f"{e}")
                tk.messagebox.showinfo(
                    title="IMPORTANT",
